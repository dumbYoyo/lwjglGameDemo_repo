...
public class Mesh {
    ...
    // not really happy with the implementation of this thing
    public Mesh(Shape shape) {
        this.vertexCount = shape.getIndices().length;
        FloatBuffer verticesBuffer = null;
        FloatBuffer textureCoordinatesBuffer = null;
        FloatBuffer normalsBuffer = null;
        IntBuffer indicesBuffer = null;
        try {
            ...
            vbos.add(vbo);
            GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
            verticesBuffer = MemoryUtil.memAllocFloat(shape.getVertices().length);
            verticesBuffer.put(shape.getVertices()).flip();
            GL15.glBufferData(GL15.GL_ARRAY_BUFFER, verticesBuffer, GL15.GL_STATIC_DRAW);
            GL20.glVertexAttribPointer(0, 3, GL11.GL_FLOAT, false, 0, 0);
            GL20.glEnableVertexAttribArray(0);
            ...
            vbos.add(tbo);
            GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, tbo);
            textureCoordinatesBuffer = MemoryUtil.memAllocFloat(shape.getTextureCoordinates().length);
            textureCoordinatesBuffer.put(shape.getTextureCoordinates()).flip();
            GL15.glBufferData(GL15.GL_ARRAY_BUFFER, textureCoordinatesBuffer, GL15.GL_STATIC_DRAW);
            GL20.glVertexAttribPointer(1, 2, GL11.GL_FLOAT, false, 0, 0);
            GL20.glEnableVertexAttribArray(1);
            ...
            vbos.add(nbo);
            GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, nbo);
            normalsBuffer = MemoryUtil.memAllocFloat(shape.getNormals().length);
            normalsBuffer.put(shape.getNormals()).flip();
            GL15.glBufferData(GL15.GL_ARRAY_BUFFER, normalsBuffer, GL15.GL_STATIC_DRAW);
            GL20.glVertexAttribPointer(2, 3, GL11.GL_FLOAT, false, 0, 0);
            GL20.glEnableVertexAttribArray(2);
            ...
            vbos.add(ibo);
            GL15.glBindBuffer(GL15.GL_ELEMENT_ARRAY_BUFFER, ibo);
            indicesBuffer = MemoryUtil.memAllocInt(shape.getIndices().length);
            indicesBuffer.put(shape.getIndices()).flip();
            GL15.glBufferData(GL15.GL_ELEMENT_ARRAY_BUFFER, indicesBuffer, GL15.GL_STATIC_DRAW);
        } finally {
            MemoryUtil.memFree(verticesBuffer);
            MemoryUtil.memFree(textureCoordinatesBuffer);
